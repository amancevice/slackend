...
const mockSecretsManager = {
  getSecretValue: (options) => {
    return {
      promise: () =>
        Promise.resolve({ SecretString: JSON.stringify(MOCK_SECRET) }),
    };
  },
};
...
describe("AWS | getApp", function () {
  before(() => {
    lambda = slackend({ secretsmanager: mockSecretsManager });
  });
...
describe("AWS | publish", function () {
  it("Succeeds with 204", function (done) {
    ...
    request(app).get("/").set("Accept", "application/json").expect(204, done);
  });

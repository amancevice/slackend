...
const app = (options = {}) => {
  return slackend(
    Object.assign({ slack: mockslack }, options)
  ).use((req, res) => res.json(res.locals.slack));
};
const err = (options = {}) => {
  return slackend(
    Object.assign({ slack: mockslackerr, signing_secret: "fake" }, options)
  ).use((req, res) => res.json(res.locals.slack));
};

const blockActions = {
  type: "block_actions",
  actions: [{ action_id: "my_action" }],
};
const callback = { type: "callback", callback_id: "my_callback" };
const viewSubmission = {
  type: "view_submission",
  view: { callback_id: "my_callback" },
};
...
describe("API | GET /health", function () {
  it("Responds OK", function (done) {
    request(app()).get("/health").expect(200, { ok: true }, done);
  });
});
...
describe("API | Verification", function () {
  ...
  it("Errors with bad timestamp", function (done) {
    request(err())
      ...
      .set(
        "x-slack-signature",
        "v0=c340868077bc902f57e4f721a98a957880c7365bea1bb1a9e6fad1a5ebc8ce9c"
      )
